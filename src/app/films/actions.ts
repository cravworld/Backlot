...
//
// Every exported action wraps its body in try/catch: an ActionError
// redirects back to the page it came from with the message in the query
// string (rendered by <ErrorBanner>), so a rejection reads as "here's
// why," not a raw dev-mode crash screen. Anything else rethrows — a real
// bug should still surface loudly, not get silently swallowed into a
// banner.
...
export async function createFilm(formData: FormData) {
  try {
    ...
    if (err instanceof ActionError) {
      redirect(`/films?error=${encodeURIComponent(err.message)}`);
    }
    throw err;
  }
}
...
export async function updateFilm(filmId: string, formData: FormData) {
  try {
    ...
    if (err instanceof ActionError) {
      redirect(`/films/${filmId}?error=${encodeURIComponent(err.message)}`);
    }
    throw err;
  }
}
...
export async function createAssignment(filmId: string, formData: FormData) {
  try {
    ...
    if (!userId || !roleId) throw new ActionError("User and role are required.");
    ...
    if (!user || !role || !film) throw new ActionError("Invalid user, role, or film for this org.");
    ...
    if (err instanceof ActionError) {
      redirect(`/films/${filmId}/assignments?error=${encodeURIComponent(err.message)}`);
    }
    throw err;
  }
}
...
export async function createCrewRole(filmId: string, formData: FormData) {
  try {
    ...
    if (!personId || !roleId) throw new ActionError("Person and role are required.");
    ...
    if (!person || !role || !film) throw new ActionError("Invalid person, role, or film for this org.");
    ...
    if (err instanceof ActionError) {
      redirect(`/films/${filmId}/crew?error=${encodeURIComponent(err.message)}`);
    }
    throw err;
  }
}
